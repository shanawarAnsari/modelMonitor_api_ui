...
import {
  getSTAvailableMonths,
  getSTMetricCards,
  getSTGroupedMetrics,
  getSTMonthlyTrends,
  getSTTrendsGroupedMetrics,
  getSTMetricTrendsCards,
...
const SetupTimeMonitor = () => {
  ...
  const [groupBy, setGroupBy] = useState("SETUP_MATRIX");
  ...
  const fetchAvailableMonths = async () => {
    try {
      const response = await getSTAvailableMonths();
  ...
  const fetchMetricCards = async () => {
    setLoading(true);
    try {
      const response = await getSTMetricCards(selectedMonth);
      setMetrics({
    ...
    try {
      ...
      const groupsParam =
        selectedGroups && selectedGroups.length > 0
          ? selectedGroups.join(",")
          : null;
      const response = await getSTGroupedMetrics(
        groupBy,
        selectedMonth,
        groupsParam
      );
    ...
    try {
      ...
      const groupsParam =
        selectedGroups && selectedGroups.length > 0
          ? selectedGroups.join(",")
          : null;
      const response = await getSTTrendsGroupedMetrics(groupBy, groupsParam);
...
export default SetupTimeMonitor;
